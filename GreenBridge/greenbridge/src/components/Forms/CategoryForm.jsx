...
const AddCategoryForm = ({ onCancel, initialCategoryData, isEdit }) => {
  ...
  const checkCategoryExists = async (name) => {
    try {
      ...
      const response = await axios.get(`https://albintomy.pythonanywhere.com/api/v1/products/category-list/`);
  ...
  const handleSubmit = async (e) => {
    ...
    const request = isEdit
      ? axios.put(
          `https://albintomy.pythonanywhere.com/api/v1/products/category-update/${initialCategoryData.id}/`,
          formData
        )
      : axios.post("https://albintomy.pythonanywhere.com/api/v1/products/category-create/", formData);
