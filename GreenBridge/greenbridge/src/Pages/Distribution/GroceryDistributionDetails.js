...
const GroceryDistributionDetails = ({ open, onClose, distribution, onStatusUpdate }) => {
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle>
                Grocery Distribution Plan Details
                {distribution?.status && (
                    <Chip 
            ...
            <DialogContent>
                ...
                {loading ? (
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <Typography variant="subtitle1">Number of Households</Typography>
                            <Typography>{distribution?.number_of_households || 'Not specified'}</Typography>
                        </Grid>

                        <Grid item xs={12} sm={6}>
                            <Typography variant="subtitle1">Persons per Household</Typography>
                            <Typography>{distribution?.persons_per_household || 'Not specified'}</Typography>
                        </Grid>

                        {distribution?.actual_households_served && (
                            <Grid item xs={12} sm={6}>
                                <Typography variant="subtitle1">Actual Households Served</Typography>
                                <Typography>{distribution.actual_households_served}</Typography>
                            </Grid>
                        )}
...
export default GroceryDistributionDetails; 