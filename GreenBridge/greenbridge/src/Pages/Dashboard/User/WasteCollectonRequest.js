...
const WasteCollectionRequest = () => {
    ...
    useEffect(() => {
        const fetchSubcategories = async () => {
            if (formData.waste_category) {
                try {
                    const response = await axios.get(`https://albintomy.pythonanywhere.com/api/v1/collection/waste-subcategories/?category=${formData.waste_category}`, {
                        headers: {
                            'Authorization': `Token ${localStorage.getItem('token')}`,
                        },
                    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post('https://albintomy.pythonanywhere.com/api/v1/collection/requests/', {
                ...formData,
